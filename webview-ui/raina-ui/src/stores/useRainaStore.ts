...
type RainaState = {
    ...
    // capability defaults captured from baseline run/artifacts
...
export const useRainaStore = create<RainaState>((set, get) => ({
    ...
    // capability defaults (pack/model) captured from baseline and reused for delta
    capabilityDefaults: {},
    setCapabilityDefaults: (d) =>
        set((s) => ({ capabilityDefaults: { ...s.capabilityDefaults, ...d } })),
    deriveCapabilityDefaults() {
        // Prefer provenance on artifacts (established by baseline run)
        ...
        // If model is still missing, try from any run record
    ...
    async switchWorkspace(workspaceId) {
        ...
        set({
            ...
            filters: { q: "", kinds: new Set(), docTypes: new Set() },
            ui: { tab: "artifacts", rightOpen: false, rightMode: "details" },
            capabilityDefaults: {}, // reset on workspace change
        });
        ...
        try {
            ...
            // Capture pack/model defaults from what we just loaded
    ...
    async refreshArtifact(artifactId) {
        ...
        // If capability defaults aren't known yet, try to derive after artifact updates
    ...
    async patchArtifact(artifactId, patch, provenance) {
        ...
        try {
            ...
            // Re-derive capability defaults just in case provenance changed
            get().deriveCapabilityDefaults();
            return "ok";
        } catch (e: any) {
    ...
    async loadRuns() {
        ...
        // If we still don't have a model default, try to take it from runs
        if (!get().capabilityDefaults.model) {
            const modelFromRun = runs.find((r) => !!r.model_id)?.model_id ?? undefined;
            if (modelFromRun) get().setCapabilityDefaults({ model: modelFromRun || undefined });
        }
    ...
    async startRun(requestBody: any) {
        try {
            ...
            const ws =
                ...
                s.wsDoc?.workspace_id ||
                s.wsDoc?.workspace?._id ||
                s.wsDoc?._id;

            if (!ws) throw new Error("No workspace selected");

            // ---- options: merge with capabilityDefaults + safe fallbacks
            const current = s.capabilityDefaults ?? {};
            const optsIn = { ...(requestBody?.options ?? {}) };
            const opts = {
                pack_key: optsIn.pack_key ?? current.pack_key ?? "svc-micro",
                pack_version: optsIn.pack_version ?? current.pack_version ?? "v1.2",
                model: optsIn.model ?? current.model ?? "openai:gpt-4o-mini",
                validate: optsIn.validate ?? true,
                dry_run: optsIn.dry_run ?? false,
            };

            // Persist these as defaults for subsequent delta runs
            set((st) => ({ capabilityDefaults: { ...st.capabilityDefaults, ...opts } }));

            // ---- IMPORTANT: ensure workspace_id is present in the BODY
            const body = {
                ...requestBody,
                workspace_id: requestBody?.workspace_id ?? ws,
                options: opts,
            };

            const res = await callHost<{ run_id: string }>({
                type: "runs:start",
                payload: { workspaceId: ws, requestBody: body },
            });

            // Refresh list quickly so the new run appears
            try { await get().loadRuns(); } catch { /* soft fail */ }
