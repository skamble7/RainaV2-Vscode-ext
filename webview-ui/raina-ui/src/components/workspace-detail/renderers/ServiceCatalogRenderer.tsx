// src/components/workspace-detail/renderers/ServiceCatalogRenderer.tsx
...
export default function ServiceCatalogRenderer({
  ...
  editable: boolean;
  onChange: (next: { services: Service[] }) => void;
}) {
  const [local, setLocal] = useState<{ services: Service[] }>(data);

  // keep local in sync when parent draft toggles off
  // (simple approach: if not editable, reset to incoming data)
  if (!editable && JSON.stringify(local) !== JSON.stringify(data)) setLocal(data);
        ...
        <div key={i} className="rounded-xl border border-neutral-800 p-3">
          ...
          <div className="grid grid-cols-2 gap-3 mt-3">
            <Field label="Owner team">
              {editable
                ? <Input value={s.owner_team ?? ""} onChange={(e) => updateService(i, { owner_team: e.target.value })} />
            ...
            <Field label="Primary datastore">
              {editable ? (
                <div className="flex gap-2">
                  <Input placeholder="type" className="w-28"
                    value={s.primary_datastore?.type ?? ""}
                    onChange={(e) => updateService(i, { primary_datastore: { ...s.primary_datastore, type: e.target.value } })} />
                  <Input placeholder="name"
                    value={s.primary_datastore?.name ?? ""}
                    onChange={(e) => updateService(i, { primary_datastore: { ...s.primary_datastore, name: e.target.value } })} />
                </div>
              ) : (
            ...
            <Field label="SLA availability">
              {editable
                ? <Input placeholder="e.g., 99.9%" value={s.sla?.availability ?? ""} onChange={(e) => updateService(i, { sla: { ...s.sla, availability: e.target.value } })} />
                : <span className="text-neutral-300">{s.sla?.availability ?? "—"}</span>}
            </Field>

            <Field label="SLA p99 latency (ms)">
              {editable
                ? <Input type="number" value={s.sla?.latency_ms_p99 ?? ""} onChange={(e) => updateService(i, { sla: { ...s.sla, latency_ms_p99: Number(e.target.value) } })} />
          ...
          <Field label="Responsibilities" className="mt-3">
            {editable ? (
              <MultiLineEditor
                ...
                onChange={(arr) => updateService(i, { responsibilities: arr })}
          ...
          <Field label="Interfaces" className="mt-3">
            {editable ? (
              <MultiLineEditor
                ...
                onChange={(arr) => updateService(i, { interfaces: arr.filter(Boolean).map((t) => {
              ...
              <div className="text-neutral-300">
                {(s.interfaces ?? []).map((it, j) => <span key={j} className="inline-block mr-2">{it.style}:{it.name}</span>)}
              </div>
            )}
          </Field>
        </div>
